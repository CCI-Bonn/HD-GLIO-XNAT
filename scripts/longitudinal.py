...
COLORS = {
    "Enhancing Tumor": (1.0, 0.0, 0.0),
    "Edema / Nonenhancing Tumor": (0.0, 1.0, 0.0),
}
    ...
    try:
        ...
        for experiment in xnat_object.experiments():
            try:
                ...
                pass
        if verbose:
            print("Downloaded CSV files for {} dates".format(len(dates)))
            ...
            if experiment.attrs.get("date").replace("-", "") == dates[-1]:
                ...
                for scan in experiment.scans():
                    if scan.attrs.get("type") == "T1":
                        scan.resource("DICOM").files()[0].get(
                            os.path.join(download_dir, "reference.dcm")
                        )
                        break
        if verbose:
            print("Downloaded reference DICOM")
        ...
        for k in volumes.keys():
            ...
            for i in range(1, len(volumes[k])):
                changes_to_previous[k].append(
                    safe_div(volumes[k][i] - volumes[k][i - 1], volumes[k][i - 1])
                )
                changes_to_baseline[k].append(
                    safe_div(volumes[k][i] - volumes[k][0], volumes[k][0])
                )
        # for k in volumes.keys():
        ...
        #             volumes[k][i] = np.inf
        if verbose:
            print("Calculated relative changes")
        ...
        for i in range(len(dates)):
            dates[i] = " ".join(
                [dates[i][6:], MONTHS[int(dates[i][4:6]) - 1], dates[i][:4]]
            )
        ...
        disclaimer = "\
            ...
            Questions: visit https://github.com/NeuroAI-HD/HD-GLIO-XNAT"
        ...
        for l, label in enumerate(volumes.keys()):
            ...
            for v, value in enumerate(volumes[label]):
                str_ = "{:.1f} $cm^3$\n{:+.0f}%\n{:+.0f}%".format(
                    value,
                    changes_to_previous[label][v] * 100,
                    changes_to_baseline[label][v] * 100,
                )
        ...
        fig.savefig(
            os.path.join(download_dir, "longitudinal.pdf"),
            bbox_extra_artists=(legend,),
            facecolor="black",
            transparent=True,
        )
        fig.savefig(
            os.path.join(download_dir, "longitudinal.jpg"),
            bbox_extra_artists=(legend,),
            dpi=100,
            facecolor="black",
            transparent=True,
        ...
        fig.savefig(
            os.path.join(download_dir, "longitudinal_t.jpg"),
            bbox_extra_artists=(legend,),
            dpi=50,
            facecolor="black",
            transparent=True,
        ...
        cmd = [
            ...
            os.path.join(download_dir, "reference.dcm"),
            os.path.join(download_dir, "longitudinal.jpg"),
            os.path.join(download_dir, "longitudinal.dcm"),
        ]
        ...
        dcm_file.SOPInstanceUID, dcm_file.SeriesInstanceUID = make_random_uids(
            dcm_file.SOPInstanceUID, dcm_file.SeriesInstanceUID
        )
        ...
        # upload
        latest_experiment.scan("9990").resource("DICOM").file("longitudinal.dcm").put(
            os.path.join(download_dir, "longitudinal.dcm"), overwrite=True
        )
        snapshot_resource = latest_experiment.scan("9990").resource("SNAPSHOTS")
        if snapshot_resource.exists():
            snapshot_resource.delete()
        snapshot_resource.file("longitudinal_t.jpg").insert(
            os.path.join(download_dir, "longitudinal_t.jpg"),
            content="THUMBNAIL",
            overwrite=True,
        )
        snapshot_resource.file("longitudinal.jpg").insert(
            os.path.join(download_dir, "longitudinal.jpg"),
            content="ORIGINAL",
            overwrite=True,
        )
        if verbose:
            print(
                "Uploaded longitudinal analysis to experiment {} of subject {}".format(
                    latest_experiment, xnat_object
                )
            )
