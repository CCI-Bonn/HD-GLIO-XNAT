...
def maybe_download_weights():
    folder_with_parameter_files = os.path.join(
        os.path.expanduser("~"), "hd_glio_params"
    )
    download_weights = False
    if not isfile(
        join(folder_with_parameter_files, "fold_0", "model_final_checkpoint.model")
    ) or not isfile(
        join(folder_with_parameter_files, "fold_0", "model_final_checkpoint.model.pkl")
    ):
